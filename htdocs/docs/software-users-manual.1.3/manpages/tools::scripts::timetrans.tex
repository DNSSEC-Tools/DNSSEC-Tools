...
\cmd{timetrans} converts time from one type of unit to another.  If any of the
units options are specified, then \cmd{timetrans} will convert those time units
into the number of seconds to which they add up.  If given the count option,
\cmd{timetrans} will convert that number of seconds into the appropriate number
...
\cmd{timetrans} is intended for use with DNSSEC-Tools, for calculating
