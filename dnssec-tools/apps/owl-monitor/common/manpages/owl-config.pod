...
B<owl-dnstimer> and B<owl-transfer> programs.  I<dnstimer-args> and
I<transfer-args> may contain spaces, but the other four all must be
positive numbers.
...
not be required to be entered.  Multiple I<ssh-user> lines may be given in the
B<owl.conf> file, and data will be transferred to/from each I<ssh-user> field
specified.  Optional parameters for <b>ssh</b> may be given on this line by
placing them after a semicolon.  This will allow disparate remote Owl hosts
to require different connection characteristics.
...
The following is an example configuration file:
    ...
    remote    ssh-user    sensor1@xyzzy.example.com;-p 6543
    remote    ssh-user    svalbard@owl.dnssec-tools.org
