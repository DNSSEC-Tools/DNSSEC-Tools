...
if [ -f "chrome.manifest" ]; then
  ...
  perl -pi -e "s/^(skin)(\s+\S+\s+\S+\s+)(\S+)$/\1\2jar:chrome\/$APP_NAME.jar!\/\3/" chrome.manifest
  perl -pi -e "s/^(locale)(\s+\S+\s+\S+\s+)(\S+)$/\1\2jar:chrome\/$APP_NAME.jar!\/\3/" chrome.manifest
#sed -E s/^\(skin\|locale\)\(\\s+\\S*\\s+\\S*\\s+\)\(.*\\/\)$/\\1\\2jar:chrome\\/$APP_NAME\\.jar!\\/\\3/ chrome.manifest
