...
void set_alarm (struct timeval	*tv, long delay)
{
					...
					struct name_server *ns)
{
	...
	temp=(struct expected_arrival *)MALLOC(sizeof(struct expected_arrival));
...
int res_io_send (struct expected_arrival *shipit)
{
	...
	*/
	...
	if (shipit==NULL) return SR_IO_INTERNAL_ERROR;
	...
	*/
	...
	{
		u_int16_t	length_n = htons((u_int16_t)(shipit->ea_signed_length));

		if((bytes_sent=send(shipit->ea_socket,&length_n,sizeof(u_int16_t),0))
				==-1)
		{
			close (shipit->ea_socket);
			shipit->ea_socket = -1;
...
int res_io_check(int transaction_id, struct timeval *next_evt)
{
	...
	if (res_io_debug) printf ("Checking at %ld\n", tv.tv_sec);
	...
	{
		while(transactions[i] && LTEQ(transactions[i]->ea_cancel_time,tv))
		{
		...
		if (transactions[i])
		{
			...
			do
			{
				if(LTEQ(temp1->ea_next_try,tv) && temp1->ea_remaining_attempts)
				{
					if (res_io_debug) printf ("SENDING\n");
					if (res_io_send (temp1)==SR_IO_SOCKET_ERROR)
					{
						...
						if (MORE_ADDRESSES (temp1))
						{
							...
							if (res_io_debug)
							{
								printf ("ERROR - SWITCHING TO NEW ADDRESS\n");
								res_print_ea (temp1);
							}
						}
						else
				...
				int signed_length, struct name_server *ns)
{
	...
	if (*transaction_id==-1)
	{
		...
		do
		{
			if (transactions[try_index]==NULL) break;
...
void res_io_set_timeout (struct timeval *timeout, struct timeval *next_event)
{
...
void res_io_collect_sockets (fd_set *read_descriptors,
...
int res_io_select_sockets (fd_set *read_descriptors, struct timeval *timeout)
{
...
int res_io_read_tcp (struct expected_arrival *arrival)
{
	...
	if (complete_read(arrival->ea_socket, arrival->ea_response, len_h) != len_h)
	{
		close (arrival->ea_socket);
		arrival->ea_socket = -1;
...
int res_io_read_udp (struct expected_arrival *arrival)
{
	int				bytes_waiting;
	...
	*/
	...
	ret_val = recvfrom(arrival->ea_socket,arrival->ea_response,bytes_waiting,
				0,&from, &from_length);

	if (ret_val==-1 ||
		memcmp (&from_in->sin_addr,&arr_in->sin_addr,sizeof(struct in_addr)) ||
...
{
	if (res_io_debug) printf ("Switching to TCP\n");
...
void res_io_read (fd_set *read_descriptors, struct expected_arrival *ea_list)
{
	...
	for (sock = 0; sock < getdtablesize(); sock++)
		...
		{
			if (res_io_debug) printf ("ACTIVITY\n");
			...
			{
				if (res_io_debug)
					printf ("Ummm, we lost the record that this socket belongs to.\nSorry.\n");
				close (sock);
				continue;
			}
			if (res_io_debug) res_print_ea (arrival);
			...
			if (arrival->ea_using_stream)
			{
				...
				if (res_io_debug) printf ("Read via TCP\n");
			}
			else
			{
				/* Use UDP */
				...
				if (res_io_debug) printf ("Read via UDP\n");
			}
			...
			*/
			...
			if(memcmp(arrival->ea_signed,arrival->ea_response,sizeof(u_int16_t))
					||
				res_quecmp (arrival->ea_signed, arrival->ea_response))
			{
				...
				if (res_io_debug)
					printf ("The the query and response ID's or q_fields don't match\n");
				FREE (arrival->ea_response);
				arrival->ea_response = NULL;
				arrival->ea_response_length = 0;
				continue;
			...
			*/
			...
			if(!arrival->ea_using_stream&&((HEADER *)arrival->ea_response)->tc)
