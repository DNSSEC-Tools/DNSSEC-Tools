...
from some name server or the request times out.  The I<libsres> library does
not automatically send out recursive queries; referral requests are also
treated as valid responses.

The memory pointed to by I<*respondent> is allocated by the I<libsres> library
...
The security options for the zone.  This can be set to either ZONE_USE_NOTHING
or ZONE_USE_TSIG.
...
The I<libsres> library also exports the following BIND symbols:
    __ns_name_unpack
    __ns_name_ntop
    __ns_name_pton
    __p_type
