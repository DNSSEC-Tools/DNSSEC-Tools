=pod

=head1 NAME

val_getaddrinfo, val_x_getaddrinfo, val_get_addrinfo_dnssec_status,
val_dupaddrinfo, val_freeaddrinfo - get DNSSEC-validated network address
and service translation

=head1 SYNOPSIS

  #include <validator.h>

  int val_getaddrinfo(const char *nodename, const char *servname,
                    const struct addrinfo *hints,
                    struct addrinfo **res);

  int val_x_getaddrinfo(const struct val_context *ctx,
                    const char *nodename, const char *servname,
                    const struct addrinfo *hints,
                    struct addrinfo **res);

  int val_get_addrinfo_dnssec_status(const struct addrinfo *ainfo);

  struct addrinfo* val_dupaddrinfo(const struct addrinfo *ainfo);

  void val_freeaddrinfo(struct addrinfo *ainfo);

=head1 DESCRIPTION

B<val_getaddrinfo()> is a DNSSEC-aware version of B<getaddrinfo(3)>.  It
performs DNSSEC validation of DNS queries.  It returns a network address value
of type I<addrinfo>.  (See B<getaddrinfo(3)> for more information on
I<addrinfo>.)

B<val_x_getaddrinfo()> performs the same function as B<val_getaddrinfo()>,
but is optimized for multiple calls.  The two routines take the same
parameters, but B<val_x_getaddrinfo()> takes an additional parameter I<ctx>,
of type I<val_context>, which passes the validation context for use in call
optimization.  The I<ctx> parameter also gives the caller more control over
the resolver and validator policies.  If a B<NULL> value is passed for the
I<ctx> parameter, the default validation context is used.  (See
B<get_context(3)> for information on creating a validation context.)
B<val_getaddrinfo()> is equivalent to calling B<val_x_getaddrinfo()> with a
B<NULL> I<ctx> parameter.

B<val_dupaddrinfo()> duplicates the I<addrinfo> structure and its auxiliary
data.  It performs a deep copy; i.e., the internal strings, arrays, and other
structures are also copied.

B<val_freeaddrinfo()> frees a I<addrinfo> structure, such as those returned
by the B<val_getaddrinfo()>, B<val_x_getaddrinfo()> and B<val_dupaddrinfo()>
functions.

B<val_get_addrinfo_dnssec_status()> extracts the DNSSEC validation status
from the returned I<addrinfo> structure.  This function must be called only
for the values returned from B<val_getaddrinfo()>, B<val_x_getaddrinfo()>,
and B<val_dupaddrinfo()> functions.

=head1 RETURN VALUES

The B<val_getaddrinfo()> and B<val_x_getaddrinfo()> functions return a value
of type I<addrinfo> on success, and B<NULL> on error.  The memory for the
returned value is dynamically allocated by these functions.  Hence, the caller
must only call the B<val_freeaddrinfo()> function on the returned value in
order to avoid memory leaks.

The B<val_get_addrinfo_dnssec_status()> function returns the result of the
DNSSEC validation.  The possible values for the DNSSEC status are given in
B<val_errors.h>.

The B<val_dupaddrinfo()> function returns a copy of the specified I<addrinfo>
structure.  The returned value must be freed using B<val_freeaddrinfo()> to
avoid memory leaks.

=head1 EXAMPLE

 #include <stdio.h>
 #include <validator.h>

 int main(int argc, char *argv[])
 {
          int dnssec_status = ERROR;
          struct addrinfo *ainfo = NULL;

          if (argc < 2) {
                  printf("Usage: %s <hostname>\n", argv[0]);
                  exit(1);
          }
 
          ainfo = val_getaddrinfo(argv[1]);

          if (ainfo) {
                  dnssec_status = val_get_addrinfo_dnssec_status(h);

                  printf("DNSSEC Status = %d [%s]\n", dnssec_status,
                         p_val_error(dnssec_status));
                  val_freeaddrinfo(h);
          }

          return 0;
 }

=head1 COPYRIGHT

Copyright 2004-2005 SPARTA, Inc.  All rights reserved.
See the COPYING file included with the DNSSEC-Tools package for details.

=head1 AUTHORS

Abhijit Hayatnagarkar

=head1 SEE ALSO

B<gethostbyname>(3)

B<get_context(3)>, B<val_duphostent(3)>, B<val_freehostent(3)>,
B<val_gethostbyname(3)>, B<val_query(3)>, B<val_x_gethostbyname(3)>,
B<val_x_query(3)>

I<p_val_error>,

http://dnssec-tools.sourceforge.net

=cut
