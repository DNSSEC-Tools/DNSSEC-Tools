...
GetOptions(\%opts, "h=s", "p=s", "f=s", "g=s", "r=i", "s", "d");
...
if ($displayFlag) {
    my $viewimage = $dtconf{'viewimage'};
    if (($viewimage) && (-e $viewimage)){
	`$viewimage $giffile`;
    }
    else {
	print STDERR "Error: Could not find a valid configuration setting for an image viewer program in /usr/local/etc/dnssec/dnssec-tools.conf\n";
	print STDERR "The image has been stored in $giffile\n";
    }
}
...
bash# ${LIBVAL}/bin/validate secure.example.com. 2>&1 | ./drawvalmap.pl -d
...
If this option is used, the generated image is displayed using an
image-viewer program as specified by the 'viewimage' setting
in the /usr/local/etc/dnssec/dnssec-tools.conf.
