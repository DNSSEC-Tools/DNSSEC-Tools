...
The path to the DNSSEC-Tools I<keyarch> command.
...
Options to pass to the I<dnssec-keygen> command.
...
The default logging level used by B<rollerd>.  The valid levels are described
in I<rollmgr.pm(3)>.
...
Options to pass to the I<named-checkzone> command.
...
Options to pass to the I<dnssec-signzone> command.

=item zonesigner

The path to the DNSSEC-Tools I<zonesigner> command.
