...
The I<DNSSEC::keyrec> module manipulates the contents of a dnssec-tools
I<keyrec> file.  I<keyrec> files contain data about zones signed by and keys
generated by the dnssec-tools.  Module interfaces exist for looking up
I<keyrec> records, creating new records, and modifying existing records.
...
The first step in using this module B<must> be to read the I<keyrec> file.
The I<keyrec_read()> interface reads the file and parses it into an internal
format.  The file's records are copied into a hash table (for easy reference
by the I<DNSSEC::keyrec> routines> and in an array (for preserving formatting
and comments.)
