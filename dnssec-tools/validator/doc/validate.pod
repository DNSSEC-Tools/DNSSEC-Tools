...
B<validate> is a diagnostic tool built on top of the DNSSEC validator.  It
...
If no options are specified and no I<DOMAIN_NAME> argument is given,
...
Print the answers and validation results.  By default, B<validate>
...
used when no name server is found, and B<validate> must do recursive
lookups itself.

=item -s, --selftest

This option can be used to specify that the application should perform its
test-suite against the I<dnssec-tools.org> test domain.  If the name servers
configured in the system B<resolv.conf> do not support DNSSEC, use the I<-r>
and I<-i> options to enable B<validate> to use its own internal recursive
resolver.
