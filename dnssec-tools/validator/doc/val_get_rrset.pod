...
name, class and type. They return the DNSSEC validation status
...
argument to val_get_rrset() if the resource record was returned after 
...
for default values (see I<libval(3)> and I<dnsval.conf> for more details on validation contexts and validation policy).
