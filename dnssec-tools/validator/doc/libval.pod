...
I<dnsval_conf_get()>, I<dnsval_conf_set()>, I<resolver_config_get()>,
I<resolver_config_set()>, I<root_hints_get()>, I<root_hints_set()> - set/get
...
Validation status for a given RRset.  This can be one of the following:
	...
	VAL_NONEXISTENT_TYPE
		No type exists for name, trusted, and
		proof present.
...
the error or success code for DNSSEC validation over the current
