...
the I<<domain_name, type, class>> tuple and to verifies and validates the
...
causes the validator to disable checking signature inception and
...
causes the validator to disable DLV processing for this query.  
This is only available if the I<libval(3)> library has
...
This option causes libval to disable EDNS0 fallback for the query.

=item B<VAL_QUERY_ITERATE>

forces libval to iteratively answer the query by querying
...
resolver (see I<dnsval.conf(3)>). These defaults may be different if 
...
Applications may also create validator context with a custom policy 
...
vc_qflags specify the default query flags for queries associated with
the context. This field is identical to the I<flags> argument in the
I<val_resolve_and_check()> function.

The vc_val_conf, vc_res_conf and vc_root_conf fields can be used to
specify alternative configuration files in place of the default files
used for the validator, resolver and root hints configuration
information.

The vc_valpol field can be used to supply a custom validator
...
as an extension or as an override, as specified by the vc_polflags
field. The vc_nslist field, similarly, can be used to supply a custom
...
The list of name servers in vc_res_conf must be specified as a
...
vc_valpol field is used for the validator configuration. Options
specified in the dnsval.conf file are ignored. 

=item B<CTX_DYN_POL_RES_OVR> 

When this flag is set only the name servers specified in the
vc_nslist field is used for the resolver configuration. Options
specified in the resolv.conf file are ignored. 

=item B<CTX_DYN_POL_GLO_OVR> 

When this flag is set only the global options set in gopt field are
used. Global options specified in the dnsval.conf file are ignored.
...
Setting a value of 1 for local_is_trusted is equivalent to specifying the
B<trust-oob-answers> option in the I<dnsval.conf> file to B<yes>. 
Similarly, setting the closest_ta_only and rec_fallback
members to 1 is equivalent to setting the closest-ta-only
and rec-fallback options to B<yes> in the B<dnsval.conf> file. 
A value of 0 is equivalent to the B<no> setting for these option.

Setting the edns0_size member to a particular value is identical to
setting the B<edns0-size> ption in the dnsval.conf file.

env_policy and app_policies can be set to one of VAL_POL_GOPT_DISABLE,
VAL_POL_GOPT_ENABLE or VAL_POL_GOPT_OVERRIDE, which
correspond directly with the B<disable>, B<enable> and
B<override> options for the env-policy and app-policy settings
in the B<dnsval.conf> file.
...
I<in additions> to the ones configured in the configuration file.


See I<dnsval.conf(3)> for more details on specifying validator policy.
...
causes the validator to set the given flag as one of the default 
query flags. 

=item B<VAL_CTX_FLAG_RESET> 

causes the validator to reset the given flag if it was set as one 
of the default query flags for the context.

=back

Answers returned by I<val_resolve_and_check()> are made available in the
I<*results> linked list.  Each answer corresponds to a distinct RRset; multiple
RRs within the RRset are part of the same answer.  Multiple answers are possible when
I<type> is I<ns_t_any> or I<ns_t_rrsig>.

Individual elements in I<*results> point to I<val_authentication_chain> linked
lists.  The authentication chain elements in I<val_authentication_chain> contain
the actual RRsets returned by  the name server in response to the query.

Validation result values returned in I<val_result_chain> and
authentication chain status values returned in each element of the
I<val_authentication_chain> linked list can be can be converted into ASCII
format using the I<p_val_status()> and I<p_ac_status()> functions respectively.
...
I<val_isvalidated()> identifies if a given
...
For an val_result_chain element that contains a valid (not NULL) val_rc_answer field, 
...
=item VAL_NONEXISTENT_NAME_NOCHAIN
...
=item VAL_NONEXISTENT_TYPE_NOCHAIN
...
Pointer to an RRset of type I<struct val_rrset_rec> obtained from the DNS response.
...
Section in which the RRset was received.  This value may be B<VAL_FROM_ANSWER>,
...
libval provides the val_log_add_optarg() function for controlling the 
verbosity and location of log message output.

The val_log_add_optarg() function takes two arguments: the first argument
...
See B<dnsval.conf(5)> for a description of syntax for these files.
