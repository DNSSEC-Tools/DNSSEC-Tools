...
I<val_create_context()>, I<val_create_context_with_conf()>,
...
I<dnsval_conf_get()>, I<resolv_conf_get()>, I<root_hints_get()> - get
the default location for the validator configuration files

I<dnsval_conf_set()>, I<resolv_conf_set()>, I<root_hints_set()> - set
the default location for the validator configuration files
...
The I<domain_name_n> parameter is the queried name in DNS wire format.
The conversion from host format to DNS wire format can be done using the
I<ns_name_pton()> function exported by the I<libsres(3)> library.
...
B<VAL_QUERY_DONT_VALIDATE> causes the validator to disable validation for this
query.  B<VAL_QUERY_NO_AC_DETAIL> causes the validator to only return the RRset 
information in val_rc_rrset of the val_result_chain structure, leaving 
the authentication chain details in val_rc_answer and val_rc_proofs empty (NULL). 
B<VAL_QUERY_NO_DLV> causes the validator to disable DLV processing for
this query.  The second flag is only available if the I<libval(3)> library has
...
resolver (see I<dnsval.conf(3)>).  The default locations of each of these
files may be changed using the interfaces I<dnsval_conf_set()>,
I<resolv_conf_set()>, and I<root_hints_set()>, respectively.  The
corresponding "get" interfaces (namely I<dnsval_conf_get()>,
I<resolv_conf_get()>, and I<root_hints_get()>) can be used to return the
...
using the I<val_create_context_with_conf()> function.
...
=item I<val_create_context()> and I<val_create_context_with_conf()>
