...
I<val_create_context()>, I<val_create_context_with_conf()> I<val_free_context()> -
...
I<p_val_status()>, I<p_ac_status()> - display validation status 
...
B<VAL_QUERY_DONT_VALIDATE>, which causes the validator to disable validation
for this query, and B<VAL_QUERY_NO_DLV>, which causes the validator to
disable DLV processing for this query. The second flag is only available
if the I<libval(3)> library has been compiled with DLV support.
...
B</etc/root.hints> provides bootstrapping information for the validator when it 
functions as a full resolver (see I<dnsval.conf(3)>).  The default locations
of each of these files may be changed using the interfaces
I<dnsval_conf_set()>, I<resolv_conf_set()>, and I<root_hints_set()>,
respectively.  The corresponding "get" interfaces (namely
I<dnsval_conf_get()>, I<resolv_conf_get()>, and I<root_hints_get()>) can be
...
basis using the I<val_create_context_with_conf()> function. 
...
Number of proof elements stored in I<val_rc_proofs>. The number cannot exceed MAX_PROOFS.
...
=item I<val_create_context()>, I<val_create_context_with_conf()>
