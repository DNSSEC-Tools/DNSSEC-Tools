...
The validation status for a given RRset. This can be one of the following:
	...
	VAL_NOTRUST
		...
		(Note that this condition should not show up since a missing trust anchor
		would show up with the appropriate error value)
...
=item I<struct val_assertion_chain> {
...
Not enough space for storing all available answers.
...
its trust status - ignore, validate or untrusted. An example is:
