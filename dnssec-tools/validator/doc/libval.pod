...
This flag forces B<libval> to iteratively lookup the query by querying
various name servers in the delegation hierarchy, instead of
requesting this information from
any caching name server that may be configured in B<dnsval.conf> when
...
assertion level and at the answer level. This flag forces B<libval> to
ignore data in both caches while trying to resolve a name.

=item B<VAL_QUERY_SKIP_ANS_CACHE>
...
using the B<val_context_store_ns_for_zone()> function.
