...
The B<val_query()> and B<val_res_query()> functions are DNSSEC-aware substitutes for B<res_query(3)>.
They perform DNSSEC validation of DNS queries.
...
The B<val_query()> function returns results in the I<resp> linked-list which
...
I<vr_response> and I<vr_length> fields are functionally similar to the
I<answer> and I<anslen> parameters in B<res_query(3)>.  Memory for the 
I<resp> linked-list is internally allocated and must be released after
a successful invocation of the function using the val_free_response() function. 
Each element in the I<resp> linked-list
...
B<VAL_SUCCESS> is returned in the I<vr_val_status> field of the
...
The B<val_res_query()> function is provided as a closer substitute
for res_query(3). It calls val_query() internally with the 
VAL_QUERY_MERGE_RRSETS flag and returns the answers in the field 
answer with length of anslen. The validation status is returned in the
field val_status. 

=head1 RETURN VALUES

The B<val_query()> function returns 0 on success.  
This function internally invokes B<resolve_n_check()>, 
and errors from this function may be returned.

The B<val_res_query()> function returns the number of bytes received on 
