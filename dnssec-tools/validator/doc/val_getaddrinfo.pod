...
B<getaddrinfo(3)>), and has an additional field I<ai_val_status> that represents
...
trusted value and the B<p_val_status()> function can be used to display this
value to the user in a useful manner (See B<libval(3)> more for information).
...
(See B<libval(3)> for information on creating a validation context.)
...
B<val_freeaddrinfo()> frees the memory allocated for a I<val_addrinfo> linked
list.
...
B<val_gethostbyname(3)>, B<val_query(3)>
