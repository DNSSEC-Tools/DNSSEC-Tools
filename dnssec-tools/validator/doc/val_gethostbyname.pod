...
B<gethostbyname2_r> functions respectively.  They perform DNSSEC validation
...
can also provide some optimization.  (See B<libval(3)> for information on
...
The B<val_istrusted()> function can be used to determine whether this 
validation status represents a trusted value and the B<p_val_status()> 
function can be used to display this value to the user in a useful 
manner (See B<libval(3)> more for information). I<val_status> must not 
be B<NULL>.
...
B<get_context(3)>, B<val_getaddrinfo(3)>, B<val_freeaddrinfo(3)>,
B<val_query(3)> 
