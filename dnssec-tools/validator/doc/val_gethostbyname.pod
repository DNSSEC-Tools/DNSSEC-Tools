...
functions perform DNSSEC validation of DNS queries.  They return a network 
host entry value of type I<struct hostent> and
are DNSSEC-aware versions of the I<gethostbyname(3)>, I<gethostbyname2(3)>,
I<gethostbyname_r()>, I<gethostbyname2_r()>, I<gethostbyaddr()> and 
I<gethostbyaddr_r()> functions respectively.  (See
I<gethostbyname(3)> for more information on type I<struct hostent>).

I<val_gethostbyname()> I<val_gethostbyname_r()>, I<val_gethostbyaddr()>
and I<val_gethostbyaddr_r()> support only IPv4 addresses.  IPv4 and IPv6 
addresses are supported by I<val_gethostbyname2()> and I<val_gethostbyname2_r()>. 
...
I<val_gethostbyname()>, I<val_gethostbyname2()> and I<val_gethostbyaddr()> set the
global I<h_errno> variable to return the resolver error code.  The reentrant
versions I<val_gethostbyname_r()>, I<val_gethostbyname2_r()> and 
I<val_gethostbyaddr_r()> use the I<h_errnop> parameter to return this value.  This ensures
...
I<gethostbyname*()> and I<gethostbyaddr*()> functions.  See the manual page for 
I<gethostbyname(3)> for more details about these parameters.
...
The I<val_gethostbyname()>, I<val_gethostbyname2()> and I<val_gethostbyaddr()>
functions return a pointer to a I<hostent> structure when they can resolve the given host name
(with or without DNSSEC validation), and NULL if data was not available.  The memory for
the returned value is statically allocated by these two functions.
Hence, the caller must not free the memory for the returned value. 

The I<val_gethostbyname_r()>, I<val_gethostbyname2_r()> and I<val_gethostbyaddr_r()> 
functions return 0 when they can resolve the given host name (with or without DNSSEC
validation), and a non-zero error-code on failure.

The I<val_gethostbyaddr()> and I<val_gethostbyaddr_r()> functions return
0 when they can resolve the given host name (with or without DNSSEC
validation), and a non-zero error-code on failure.

The I<val_status> parameter gives an indication for trustworthiness of data. If the returned
I<hostent> structure is NULL, this value gives an indication of whether the non-existence of
...
I<val_getaddrinfo(3)>, I<val_query(3)> 
